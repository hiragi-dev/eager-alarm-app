...
**開発時のみのテスト用UI**: 実際に停止地点まで移動しなくても上記の到達検知〜ポップアップの
フローを試せるよう、「アラームを止める」タブの停止方法カードに「この停止方法の位置まで移動した
ことにする」ボタンを用意しています（`process.env.NODE_ENV`判定で本番ビルドからは完全に除去されます）。
`src/contexts/LocationProvider.tsx`の`setSimulatedPosition`で疑似座標を設定するだけで、
`ArrivalStopBridge.tsx`/`StopAlarmControl.tsx`側は本物の到達と区別せず同じ到達判定・送信・確認
ロジックを通ります。疑似座標は`useLocation().currentPosition`（実GPS専用）とは別の
`simulatedPosition`として保持しており、「停止方法」の地図選択など他画面の現在地表示には
一切影響しません。また、停止を確認できた時点で自動的に解除されるため、手動での解除操作は不要です。
